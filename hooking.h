...
namespace menu {
	...
	static void run(ucanvas* canvas) {
		if (render::show_menu) {
			...
			if (ZeroGUI::Window("novoline [mrkz updated this] sell = gay", &pos, fvector2d{ 500.0f, 900.0f }, render::show_menu, width, height)) {
				...
				{
					ZeroGUI::Text("Updated by mrkz (thanks for signing my dll)");
				}
			}
		}
	}
...
void novoline_main(ugameviewportclient* viewport, ucanvas* canvas) {
	...
	if (settings::miscellaneous::noreload) {
		if (local_pawn) {
			...
			if (CurrentWeapon)
			{
				...
				auto Mesh = *(uintptr_t*)(local_pawn + offsets::offset_get_pawn_mesh); // 0x318 = Mesh
	...
	if (settings::miscellaneous::speedhack) {
		...
		if (local_pawn) {
		{
			auto CurrentWeapon = local_pawn->get_current_weapon();
			if (CurrentWeapon) {
				*(float*)(CurrentWeapon + offsets::LastFireTime) = settings::miscellaneous::rapidfire;
				*(float*)(CurrentWeapon + offsets::LastFireTimeVerified) = settings::miscellaneous::rapidfirevalue;
			}
		}
	}
